...
import { 
  ...
  Map, Landmark, ShieldAlert, Trophy, GraduationCap, Globe, Users, Megaphone
} from 'lucide-react';
import Link from 'next/link'; 
import { useNews } from '@/context/NewsContext';
import { supabase } from "@/lib/supabase"; // 🔥 PENTING UNTUK TARIK HARGA 🔥
...
export default function HomePage() { 
  ...
  // 🔥 STATE UNTUK MENYIMPAN PENGATURAN HARGA IKLAN 🔥
  ...
  // 🔥 TARIK DATA HARGA IKLAN SAAT HALAMAN DIBUKA 🔥
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <header className="fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-3">
          ...
          <div className="h-12 flex items-center justify-between">
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <button onClick={() => handleCategoryClick("Home")} className="flex items-center gap-1.5 group cursor-pointer">
                <div className="bg-gradient-to-tr from-blue-600 to-blue-400 p-1.5 md:p-1 rounded-md text-white group-hover:rotate-12 transition-transform shadow-md"><Zap className="w-3.5 h-3.5 md:w-3 md:h-3 fill-white" /></div>
                <span className={`text-[15px] md:text-base font-black italic tracking-tighter ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>SULTRA<span className="text-blue-600">FIKS</span></span>
              </button>
            </div>
          ...
          <div className="md:hidden pb-2 px-1">
            <div className={`rounded-xl px-3 py-1.5 flex items-center border ${isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-100/50 border-slate-200'}`}>
              <Search className={`w-3.5 h-3.5 mr-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-400'}`}/>
              <input className={`bg-transparent border-none outline-none text-xs w-full ${isDarkMode ? 'text-slate-200 placeholder:text-slate-500' : 'text-slate-700 placeholder:text-slate-400'}`} placeholder="Cari topik berita..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
              {searchQuery && <button onClick={() => setSearchQuery("")}><X className="w-3.5 h-3.5 text-red-500" /></button>}
            </div>
          </div>

          <div className="overflow-x-auto no-scrollbar flex gap-2 md:gap-4 pb-2 pt-2 items-center px-1">
            {categories.map((cat) => {
              const config = categoryConfig[cat];
              const IconComponent = config.icon;
              const isActive = activeCat === cat && !searchQuery && activeCat !== "Indeks"; 
              return (
                <button key={cat} onClick={() => handleCategoryClick(cat)} className="flex flex-col items-center gap-1.5 md:gap-1.5 group shrink-0 px-1 md:px-2">
                  <div className={`w-9 h-9 md:w-11 md:h-11 rounded-[10px] flex items-center justify-center transition-all duration-300 relative overflow-hidden ${isActive ? `bg-gradient-to-tr ${config.color} text-white shadow-sm border-0` : (isDarkMode ? 'bg-slate-800 text-slate-400 border-slate-700 border hover:bg-slate-700 hover:text-white' : 'bg-slate-50 text-slate-500 border-slate-200 border hover:bg-white hover:text-blue-600')}`}>
                    <IconComponent className={`w-4 h-4 md:w-5 md:h-5 relative z-10 transition-transform ${isActive ? 'scale-110' : ''}`} />
                  </div>
                  <span className={`text-[9px] md:text-[10px] font-bold tracking-wide transition-colors ${isActive ? 'text-blue-500' : (isDarkMode ? 'text-slate-400 group-hover:text-slate-200' : 'text-slate-500 group-hover:text-slate-800')}`}>{cat}</span>
                </button>
              )
            })}
          </div>
      ...
      <main className="max-w-7xl mx-auto px-3 md:px-4 pt-[160px] md:pt-32 pb-5 md:pb-8 space-y-6 md:space-y-10">
              ...
              {activeCat === "Indeks" ? (
                ...
                <div className="space-y-8 md:space-y-10">
                  ...
                  {categories.filter(c => c !== "Home").map((catName) => {
                    const catNewsList = allNewsSorted.filter(n => n.category === catName).slice(0, 2);
      ...
      <AnimatePresence>
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              <div className="bg-gradient-to-tr from-blue-600 to-indigo-600 p-6 flex flex-col items-center justify-center relative">
                <button onClick={() => setIsAboutOpen(false)} className="absolute top-4 right-4 p-2 bg-white/20 hover:bg-white/30 rounded-full text-white transition-colors"><X className="w-4 h-4" /></button>
        ...
        {/* 🔥 MODAL CONTACT ADMIN DENGAN HARGA DARI DATABASE 🔥 */}
        {isContactOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div 
              initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }}
            ...
            >
              <div className="bg-gradient-to-tr from-amber-500 to-orange-600 p-6 flex flex-col items-center justify-center relative">
                <button onClick={() => setIsContactOpen(false)} className="absolute top-4 right-4 p-2 bg-white/20 hover:bg-white/30 rounded-full text-white transition-colors"><X className="w-4 h-4" /></button>
